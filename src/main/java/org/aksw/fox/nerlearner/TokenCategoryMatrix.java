...
public class TokenCategoryMatrix {
  ...
  public void setFound(final Entity entity, final String splitregText) {
    ...
    final String[] split = entity.getFullText().split(splitregText);
    ...
    for (String s : split) {
      ...
      if (!s.isEmpty()) {
        ...
        if (tokenIndex > -1) {
          ...
          values[tokenIndex][categories.indexOf(entity.getCategory())] = true;
        } else {
          logger.error("token not found: " + s);
        }
      }
    }
    //Amon =>matriz ta errada     
      /*for (Token token : entity.getWords()){
        ...
        if (tokenIndex > -1){
          ...
          if(categories.indexOf(token.getType())>-1){
            values[tokenIndex][categories.indexOf(token.getType())] = true;
          } else {
            logger.error("TokenIndex out of category="+tokenIndex);
          }
          
        } else {
          logger.error("token not found: " + token.getText());
        } 
      }*/
  }
