...
public class Fox extends AFox {
  ...
  protected Set<Entity> doNER() {
    ...
    for (final Entity entity : entities) {
      if (wordEntityMap.get(entity.getFullText()) == null) {
        wordEntityMap.put(entity.getFullText(), entity);
      } else {
        if (LOG.isDebugEnabled()) {
          LOG.debug("We have a duplicate annotation and removing those: " + entity.getFullText() + " "
              + entity.getCategory() + " " + wordEntityMap.get(entity.getFullText()).getCategory());
        }
        wordEntityMap.remove(entity.getFullText());
      }
    }
    // remove
  ...
  protected Set<Entity> doNERLight(final String name) {
    ...
    for (final Entity entity : entities) {
      for (final Integer i : FoxTextUtil.getIndices(entity.getFullText(),
  ...
  protected void html(final Set<Entity> entities, final String input) {
    ...
    for (final Integer index : startIndices) {
      ...
      if ((entity.getUri() != null) && !entity.getUri().trim().isEmpty()) {
        ...
        html += "<a class=\"" + entity.getCategory().toLowerCase() + "\" href=\"" + entity.getUri()
            + "\"  target=\"_blank\"  title=\"" + entity.getCategory().toLowerCase() + "\" >"
            + entity.getFullText() + "</a>";
        last = index + entity.getFullText().length();
      } else {
        LOG.error("Entity has no URI: " + entity.getFullText());
      }
    }
